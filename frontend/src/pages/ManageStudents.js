...
export default function ManageStudents() {
  ...
  const deleteStudent = async (roll_no) => {
    if (!window.confirm(`Delete student with Roll No ${roll_no}? This will remove related attendance.`))
      return;
    try {
      const res = await api.delete(`/api/students?roll_no=${encodeURIComponent(roll_no)}`);
      await load();
      alert(res.data?.message || "Deleted");
    } catch (err) {
      console.error("Delete failed:", err);
  ...
  // Filter students by class + partial roll search
  const filtered = students.filter((s) => {
    if (filterClass && s.class !== filterClass) return false;
    if (searchRoll) {
      const qNorm = String(searchRoll).trim().replace(/^0+(?=\d)/, "");
      const sRollNorm = String(s.roll_no ?? "").replace(/^0+(?=\d)/, "");
      if (!sRollNorm.includes(qNorm)) return false;
    }
    return true;
  });
  ...
  const header = (label, key) => (
    <th className="table-head-cell sortable" onClick={() => handleSort(key)} role="button">
      <span className="col-label">{label}</span>
      <span className="sort-indicator">{sortBy === key ? (sortDir === "asc" ? "▲" : "▼") : "↕"}</span>
    </th>
  );
  ...
  return (
    <div className="manage-students">
      <div className="controls">
        ...
        <label className="controls-label"><b>Search Roll No:</b></label>
        <input className="filter-select" placeholder="enter roll no" value={searchRoll} onChange={(e) => setSearchRoll(e.target.value)} />
        ...
        <table className="students-table" aria-label="Students table">
          ...
          <tbody>
            {displayed.map((s) =>
              editingRoll === s.roll_no ? (
                <tr key={s.roll_no}>
                  <td><input className="form-input small" value={form.roll_no} disabled /></td>
                  <td><input className="form-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></td>
                  <td><input className="form-input small" value={form.class} onChange={(e) => setForm({ ...form, class: e.target.value })} /></td>
                  <td><input className="form-input" value={form.father_name} onChange={(e) => setForm({ ...form, father_name: e.target.value })} /></td>
                  <td><input className="form-input small" type="date" value={form.date_of_joining} onChange={(e) => setForm({ ...form, date_of_joining: e.target.value })} /></td>
                  <td><input className="form-input small" value={form.contact_no} onChange={(e) => setForm({ ...form, contact_no: e.target.value })} /></td>
                  <td><input className="form-input" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} /></td>
                  <td>
                    <button className="btn" onClick={saveStudent} style={{ marginRight: 6 }}>Save</button>
                    <button className="btn" onClick={cancelEdit}>Cancel</button>
                  </td>
                ...
                <tr key={s.roll_no}>
                  <td>
                    <Link to={`/students/${s.roll_no}`}>{s.roll_no}</Link>
                  </td>
                  <td>{s.name}</td>
                  <td>{s.class}</td>
                  <td>{s.father_name}</td>
                  ...
                  <td>
                    <button className="btn" onClick={() => startEdit(s)} style={{ marginRight: 6 }}>Edit</button>
                    <button className="btn" onClick={() => deleteStudent(s.roll_no)}>Delete</button>
                  </td>
                </tr>
              )
            )}
